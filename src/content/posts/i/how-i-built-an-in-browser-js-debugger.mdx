---
title: "Thread Syncronization for Web Apps: How I Built An In Browser Javascript Debugger"
description: "The story of how I built a Javascript in-browser debugger using Atomics and SharedArrayBuffer"
short_desc: "The story of how I built a Javascript in-browser debugger using Atomics and SharedArrayBuffer"
...
Before I get started, let me show you what I will be talking about in this article. 
...
What you just played with is a widget that sits at the core of [primiipasi.info](https://primiipasi.info) and is a niftly little Javascript "debugger" meant to help kids visualize how code...runs. The idea was to give them something to interact with so they can see how their code actually executes and how data is changed at runtime.

## The requirements
This would not be a full blown debugger but a widget that could:
- _step through code_ line by line
- show _values of variables_ and signal visually when they get updated
- show the variables' _scopes_ during execution

## The challenges
The first challenge was to figure out how to parse the code so I could _track the variables/functions_ being updated/called. For this I wanted to be able to inject myself in different areas of the code so I could basically know what was going on in the script. [Esprima](https://esprima.org/index.html) to the rescue.
...
Even if the code would now be running in a separate thread from the UI's, I still needed to be able to pause/start its execution to create the step-over behaviour of a debugger. For this, one would need a _thread synchronization_ mechanism which in Javascript can be achieved using [SharedArrayBuffer](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/SharedArrayBuffer) -- which proved to be a can of worms that I will talk about later.

## General architecture
![](/images/posts/38a28092-71a9-437e-94f1-555470daf405.png)

What is obvious from the diagram is that the app runs within two threads: 
- the _UI thread_ containing the interface and handling the user interaction
- the _worker thread_ handling the processing and execution of the debugged code

A brief explanation of the main boxes:
...
- ```CodeProcessor``` is the class responsible for tokenizing the code using [esprima](https://esprima.org/index.html) and generating the cookied code (the one containing the cookie functions responsible for determining the state of the program)
...
- ```CodeExecutorProxy``` is just a wrapper over the ```CodeExecutor``` which is meant to be used from the main thread to communicate with it
- ```CodeDebugger``` is an orchestrator of all the messages/event going from the UI to the worker thread and vice versa
...
As mentioned earlier, I've used *esprima* to tokenize the code and what it does is to return a very nice tree structure containing all the elements of the script alongside their positions in the source code.
...
### Cookies 🍪
Knowing the code constructs and their source code positions, the next step was to figure out which ones are interesting to me considering the requirements I have (to track the variable assignments, function call, scopes, etc.) and _inject specifically taylored functions (cookies)_ for each that will act as notifiers when the code reaches their execution.

Using the code snippet above, let's take line ```6``` as an example. 

![](/images/posts/d9c5a839-6fa8-411d-b50d-441a670f9382.png)

The ```let x = 1;``` variable declaration and initialization is represented by esprima with an object of type ```VariableDeclarator``` and alongside it's properties, it also contains the ```range``` field with the position in the source code, from index ```52 to 62```. I would now know that between these positions in the source code there is an assignment happening.
...
- ```funcWrap(...)``` wrapper function over all function calls

With this said, the initial code after injecting the cookies became:
```ts
...
This sounds straightforward but a lot of heavy lifting was required to figure out what and where to inject each cookie. The code can be seen here ▶️ [src/code-processor.ts](https://github.com/24rush/algo-vis/blob/develop/src/code-processor.ts)

## Code execution
Executing the code involved passing the processed source code from the main thread to the worker and then sending the command to execute it.

### The handshake step
This step involved sending three pieces of information:
- the processed source code
- a reference to the shared memory (SharedArrayBuffer) to later by used for synchronization between the UI thread and the worker
- the 'execute' signal to start executing the code

For passing messages to a spawned worker thread, the [Worker::postMessage](https://developer.mozilla.org/en-US/docs/Web/API/Worker/postMessage) function was used as follows:

The ```CodeExecutorProxy``` calls the ```Worker.postMessage```:
```js
...
on the other side (in the worker thread), the receiving ```CodeExecutor``` class receives it through the [```self.onmessage```](https://developer.mozilla.org/en-US/docs/Web/API/DedicatedWorkerGlobalScope/message_event) callback:
```js
...
In the ```CodeDebugger``` class the complete handshake step is implemented as follows:
```js
this.codeExecProxy.init(); // send shared memory reference
this.codeExecProxy.setSourceCode(this.codeProcessor.getCode()); // pass the source code
this.codeExecProxy.execute(); // trigger the execution
```

and because _postMessage_(s) are just events in the queue of the worker thread that were send from the main thread, they will also be processed in the same order in the worker thread.

## Stepping over in code
The whole point of the widget was to offer the possibilty to execute the code _line by line_ so users can see how each code line affects the state of the program.

The way this was implemented was using a blocking cookie function (```markcl``` more precisely) that would halt the execution of the script/thread until it got the signal to proceed from the user/main thread. This synchronization mechanims is hanled in Javascript by [Atomics](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Atomics) and [SharedArrayBuffers](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/SharedArrayBuffer).

All this would have not been possible without the ```Atomics.wait``` function which uses a _SharedArrayBuffer_ to verify if an index position from that buffer is of a certain value and if so it sleeps until either a wake-up notification is received or a time-out.

### The synchronization mechanism:

Let's take this 2 line example:
...
2. Code is _eval-ed_ and execution reaches ```markcl(6);``` with the following implementation:
    ...
    What is happening:
    - a message is sent back to UI thread notifying what the current code line is so it can place the highlighted line in the code editor
    - ```Atomics.store(sharedMem, 0, Wait)``` is called placing value ```Wait``` at index 0 of the shared memory. 
    - ```Atomics.wait(sharedMem, 0, Wait)``` is called signifying that the current worker thread will sleep as long as value Wait will be stored at position 0 in the shread memory - _it basically blocks itself_
...
5. Execution moved forward with the code below:
    ```js
      let x = 1;;setVar('x', x);
      markcl(7);
   ``` 
...
## Updating the UI
Whenever one of the cookie functions gets executed, a message is send back to the main thread and some processing is done to figure out what happened after which a message is re-sent to update the UI in a particular way.

![](/images/posts/bc640bc0-eaf1-47f9-8a31-8d42b6d1f715.png)

Let's take for example the creation of a new scope. 

1. The ```startScope(<scopeName>)``` cookie function is executed in the worker thread which sends a message to the parent thread (main thread):
...
2. The main thread puts it on the ```NotificationBus```:
    ```js
    ...
    and the ```NotificationBus``` calls all subscribers to the ```startScope``` message:
    ```js
    startScope(scopeName: string) {
...
4. This message is then received by the widget's UI and handled in the form of generating a HTML fragment for the new scope and placing it in the corresponding location in the DOM:
...
- keep track of active variables and their scope (identifying the scope of a variable was the most challenging part as the only information I would get from the cookies was that a variable with a name was set a certain value and then I would need to figure out to the variable in which scope it was referring to)

The code for the Magic box can be found ▶️ src/